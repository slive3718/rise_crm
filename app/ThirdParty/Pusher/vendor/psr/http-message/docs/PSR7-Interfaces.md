...
| `getHeaders()`                     | Retrieve all HTTP Headers               | [Request Header List](http://en.wikipedia.org/wiki/List_of_HTTP_header_fields#Request_fields), [Response Header List](http://en.wikipedia.org/wiki/List_of_HTTP_header_fields#Response_fields)      |
...
| Method Name                        | Description | Notes |
|------------------------------------| ----------- | ----- |
| `getRequestTarget()`                | Retrieves the message's request target              | origin-form, absolute-form, authority-form, asterisk-form ([RFC7230](http://www.rfc-editor.org/rfc/rfc7230.txt)) |
| `withRequestTarget($requestTarget)` | Return a new message instance with the specific request-target |      |
| `getMethod()`                       | Retrieves the HTTP method of the request.  |  GET, HEAD, POST, PUT, DELETE, CONNECT, OPTIONS, TRACE (defined in [RFC7231](http://tools.ietf.org/html/rfc7231)), PATCH (defined in [RFC5789](http://tools.ietf.org/html/rfc5789)) |
