<?php //print_r(get_setting("invoice_footer"));exit;?><div style=" margin: auto;">
    <?php
    $colspan = 3;
    $show_taxable = false;
...
<table class="table-responsive" style="width: 100%;">
    <tr style="font-weight: bold; background-color: <?php echo $color; ?>; color: #fff;  ">
        <th style="text-align: center; width: 45%; border-right: 1px solid #eee;"> <?php echo app_lang("item_desc"); ?> </th>
        <th style="text-align: center;  width: <?php echo $show_taxable ? '12%' : '15%'; ?>; border-right: 1px solid #eee;"> <?php echo app_lang("quantity"); ?></th>
        <th style="text-align: center;  width:<?php echo $show_taxable ? '12%' : '20%'; ?>; border-right: 1px solid #eee;"> <?php echo app_lang("rate"); ?></th>
        <?php if ($show_taxable) { ?>
            <th style="text-align: center; width: 12%;  border-right: 1px solid #eee; "> <?php echo app_lang("taxable"); ?></th>
        <?php } ?>
        <th style="text-align: center;  width: <?php echo $show_taxable ? '19%' : '20%'; ?>; "> <?php echo app_lang("total"); ?></th>
    </tr>
    <?php
    if($invoice_items):
    foreach ($invoice_items as $item) { ?>
        <tr style="background-color: <?php echo $item_background; ?>;">
            <td style="width: 45%; border: 1px solid #fff; padding: 10px;"><?php echo $item->title; ?>
                <br />
                <span style="color: #888; font-size: 90%;"><?php echo custom_nl2br($item->description ? strip_tags($item->description) : ""); ?></span>
            </td>
            <td style="text-align: center; vertical-align:middle; height:10px; width: <?php echo $show_taxable ? '12%' : '15%'; ?>; border: 1px solid #fff;"> <?php echo $item->quantity . " " . $item->unit_type; ?></td>
            <td style="text-align: center; vertical-align:middle; height:10px; width: <?php echo $show_taxable ? '12%' : '20%'; ?>; border: 1px solid #fff;"> <?php echo to_currency($item->rate, $item->currency_symbol); ?></td>
            <?php if ($show_taxable) { ?>
                <td style="text-align: center; vertical-align:middle; height:10px;width: 12%; border: 1px solid #fff; "> <?php echo $item->taxable ? app_lang("yes") : app_lang("no"); ?></td>
            <?php } ?>
            <td style="text-align: center; vertical-align:middle; height:10px; width: <?php echo $show_taxable ? '19%' : '20%'; ?>; border: 1px solid #fff;"> <?php echo to_currency($item->total, $item->currency_symbol); ?></td>
        </tr>
    <?php 
    } endif ?>
    <tr>
...
<?php } ?>
<span style="color:#444; line-height: 25px;">
    <?php $settingsFooter = get_setting("invoice_footer"); 
