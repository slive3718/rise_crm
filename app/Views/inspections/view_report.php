        ...
        <div class="card-body">
            <div class="page-body">
                <div class="container my-4">
                    ...
                    <div class="card bg-none">
                        ...
                        <div class="card-body">
                            <div class="text-center mb-3">
                                <img src="<?= $LOGO_URL ?>" alt="Company Logo" class="img-fluid" style="max-height: 100px;">
                    ...
                    <div class="card" style="background-color: #b3c1e0;">
                        <div class="form-section">
                            ...
                            <div class="form-section-header px-4 pt-3" data-bs-toggle="collapse" data-bs-target="#section-flagged-items" aria-expanded="false" aria-controls="section-flagged-items">
                                Flagged Items
                                <span data-feather="bell" class="icon-16 ms-3 text-danger" title="Edit" ></span>
                                <span class="text-danger" id="flagged_items_count">0</span>
                            </div>
                                ...
                                <?php foreach ($sections as $section_name => $fields): ?>
                                    <?php foreach ($fields as $field): ?>
                                        <?php if ($field['field_type'] == 'radio'): ?>
                                            <?php
                                            // Check if there are any flagged options
                                            $hasFlaggedOptions = false;
                                            foreach (json_decode($field['field_options'], true) as $option) {
                                                if (isset($option['flags']) && $option['flags'] == '1' && $field['value'] == $option['label']) {
                                                    $hasFlaggedOptions = true;
                                                    $hasFlaggedOptionsCount ++;
                                                    break;
                                                }
                                            }
                                            ?>
                                            <?php if ($hasFlaggedOptions): ?>
                                                <div class="form-group">
                                                    <!-- Radio buttons styled with colors -->
                                                    <div class="card border-0 p-3 ">
                                                        <div class="mb-2">
                                                            <h5 class="mb-0"><?= htmlspecialchars($field['field_label']) ?></h5>
                                                        </div>
                                                        <div class="row">
                                                            <?php foreach (json_decode($field['field_options'], true) as $option): ?>
                                                                <?php if ($field['value'] == $option['label'] && isset($option['flags']) && $option['flags'] == '1'): ?>
                                                                    <div class="col-12">
                                                                        <?php if( $field['value'] == $option['label'] ): ?>
                                                                            <badge class="badge py-1 px-4 text-start"
                    ...
                    <script>
                        $(function(){
                            let flagged_items_count  = `<?=$hasFlaggedOptionsCount?>`
                            $("#flagged_items_count").text(flagged_items_count)
                            $("#flagged_items_count").html(flagged_items_count)
                        })
