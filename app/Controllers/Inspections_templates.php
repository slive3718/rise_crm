...
class Inspections_templates extends Security_Controller
{
        ...
        try {
            ...
            foreach ($sections as $section) {
                ...
                foreach ($section['fields'] as $field) {
                    ...
                    // If the flags array is shorter than the options array, fill in with 0s\
                        ...
                        if (isset($field['options']) && is_array($field['options'])) {
                            foreach ($field['options'] as $optionIndex => $option) {
                                $fieldOptionsArray[] = [
                                    ...
                                    'flagged' => $field['flagged'][$optionIndex] ? '1': '0',// Set '1' for flagged, '0' otherwise
                                ];
                            }
                        }
    ...
    public function update() {
        ...
        $this->Inspections_fields_model->where('template_id', $template_id)->delete();

        try {
            // Load the models
            $inspectionFieldsModel = $this->Inspections_fields_model;
            // Get sections and fields data
            $sections = $this->request->getPost('sections');
...
//                    print_r($section['fields']);exit;
                    ...
                    foreach ($section['fields'] as $field) {
                        ...
                        $flags = isset($field['flagged']) && is_array($field['flagged']) ? $field['flagged'] : [];
                        $submittedFlagged = $field['flagged'] ?? []; // Only contains checked values
                        // If the flags array is shorter than the options array, fill in with 0s\
                        if (isset($field['options'])) {
                            for ($i = 0; $i < count($field['options']); $i++) {
                                if (!isset($flags[$i])) {
    ...
    public function get_template_field_data(){
        $templateModel = new Inspections_templates_model();
        $view_data['templates'] = $templateModel->first();
        ...
        return $this->template->view("inspections/edit_inspection_form_fields", $view_data);
    }
    ...
    public function list() {
        $templateModel = $this->Inspections_model;
