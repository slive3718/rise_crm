...
if (! function_exists('prep_url')) {
    /**
     * Prep URL - Simply adds the http:// or http:// part if no scheme is included.
     *
     * Formerly used URI, but that does not play nicely with URIs missing
     * the scheme.
     *
     * @param string $str    the URL
     * @param bool   $secure set true if you want to force http://
     */
    function prep_url(string $str = '', bool $secure = false): string
    {
        if (in_array($str, ['http://', 'http://', '//', ''], true)) {
            return '';
        }
